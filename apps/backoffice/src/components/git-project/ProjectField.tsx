...
export default function ProjectField({ repo, _id }: RepositoryPageProps) {
  ...
  return (
    <div
      className={`w-full border-b-2 border-gray-300 p-3 flex justify-between ${_id === 0 ? "border-t-2" : ""}`}
    >
      ...
      <div>
        <button
          ...
          className={`w-16 h-6 ${repoSelected.repo_id === _id ? "bg-blue-800" : "bg-customColor"} text-white text-xs rounded-md font-medium`}
        >
          {repoSelected.repo_id === _id ? "Selected" : "Import"}
        </button>
      </div>
    </div>
  );
