...
export default function Dropdown() {
  ...
  const [selectedBranch, setSelectedBranch] = useState<string>(
    repoSelected.repo_info.default_branch,
  );
  const { changeValue } = useEnvContext();

  const getCommit = async () => {
    try {
      const infoRepo = repoSelected.repo_info.svn_url.split(
        "https://github.com/",
      )[1];
      await axiosAuth
  ...
  useEffect(() => {
    ...
    const getBranchesList = async () => {
      try {
        await axiosAuth
          .post("/v1/github-app/get-data", {
            url: `${repoSelected.repo_info.branches_url}`,
          })
          .then((response) => response.data)
          .then((data) => {
            setBrachesList(data);
  ...
  return (
    <Menu
      as="div"
      className="relative w-[8.5rem] sm:w-36 inline-block text-left"
    >
