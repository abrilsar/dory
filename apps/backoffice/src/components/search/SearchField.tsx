...
export default function SearchField() {
  ...
  return (
    <div className=" h-7 flex items-center rounded mx-auto border bg-white border-gray-400 focus-within:border-purple-600 py-1 sm:w-min-40 lg:w-2/4">
      <svg
        className="w-5 h-5 rounded-full ml-3 text-gray-500"
      ...
      <input
        className="w-full h-6 border-none rounded focus:ring-0 focus:outline-none text-xs"
        type="text"
        placeholder="Search ..."
        onChange={(event) => {
          setText(event.target.value);
        }}
        onClick={() => setRepoSelected(INITIAL_VALUE_REPO)}
      />
    </div>
  );
}
