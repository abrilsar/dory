...
export default function RepositoryPage() {
  ...
  const handleCheckRepo = async () => {
    ...
    if (repoSelected.repo_id === -2) {
      ...
      const response = await axiosAuth.post("/v1/github-app/get-data", {
        url: `/repos/${endPoint}`,
      });
        ...
        for (const id of installation_id) {
          const response = await axiosAuth
            .post("/v1/github-app/get-data", {
              url: `/user/installations/${id}/repositories`,
            })
            .then((response) => response.data)
            .then((data) => data.repositories)
            .then((repos) => newRepoList.push(...repos));
        }
  ...
  return (
    <div className="flex flex-col justify-center items-center mt-6 overflow-y-auto h-96">
      <p className="text-lg font-semibold w-full sm:w-3/4 md:w-2/3 lg:w-2/4 xl:w-2/4 text-center justify-start items-start py-4">
        Import Repository
      </p>
      ...
      <div className="w-11/12 sm:w-3/4 md:w-[34rem] flex-col overflow-y-scroll border-1 border-gray-700 h-96">
        ...
        {isLoading ? (
          ...
          <Illustration
            ...
            size={"w-20 h-20"}
            alt={"No Projects"}
            text1={"Looks like you don't have any repos"}
            text2={""}
          />
        ) : filterList(repoList, "github").length === 0 ? (
          <div className="justify-center content-start">
            <Illustration
              pt={"pt-4"}
              size={"w-20 h-20"}
              alt={"Search Not Found"}
              text1={"Result not found"}
              text2={""}
            />
          </div>
        ) : (
        ...
        <div className="mr-4 px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
          ...
          <dd className="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0 relative">
            ...
            <p
              ...
              className={`mt-2 text-xs ${repoSelected.repo_id === -2 ? "text-green-500" : "text-red-500"} font-medium absolute top-full`}
            >
              {invalidText}
            </p>
          </dd>
