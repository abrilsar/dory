...
export default function GitImportPage() {
  ...
  const getListInstallation = (data: any, installation: any) => {
    ...
    data.installations.map((element: any) => {
      if (!newList.includes(element.id)) newList.push(element.id);
    });
    ...
    const checkInstallation = async () => {
      ...
      if (data.total_count !== 0) {
        const installation = data.installations.find(
          (installation: { app_id: number; account: { id: string } }) =>
            installation.app_id === 883977 &&
            installation.account.id.toString() === session?.user._id,
        );
